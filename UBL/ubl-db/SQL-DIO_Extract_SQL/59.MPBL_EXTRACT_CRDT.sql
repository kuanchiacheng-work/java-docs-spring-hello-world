SELECT 'CRDT' crdt, RPAD (TRIM (ba.acct_id), 12, ' ') billing_arrangement_id,
       RPAD (TRIM (ba.acct_id), 12, ' ') cm_pay_channel_id,
       TO_CHAR (TRUNC (fet1_customer_credit.credit_date),
                'YYYYMMDD'
               ) credit_date,
       RPAD (TRIM (fet1_customer_credit.amount), 23, ' ') amount,
       RPAD (TRIM (fet1_customer_credit.charge_code), 15, ' ') charge_code,
       RPAD (TRIM (fet1_customer_credit.chg_revenue_code),
             6,
             ' '
            ) chg_revenue_code,
       RPAD (TRIM (fet1_customer_credit.tax_amount), 20, ' ') tax_amount,
       RPAD (TRIM (fet1_customer_credit.credit_reason), 10,
             ' ') credit_reason,
       RPAD (TRIM (fet1_customer_credit.credit_id), 12, ' ') credit_id,
       RPAD
          (TRIM (fet1_customer_credit.l9_service_receiver_type),
           1,
           ' '
          ) l9_service_receiver_type,
       RPAD
          (TRIM (fet1_customer_credit.l9_service_receiver_id),
           12,
           ' '
          ) l9_service_receiver_id,
       RPAD (TRIM (DECODE (NVL (fet1_customer_credit.invoice_id, 0),
                           0, fet1_customer_credit.transaction_id,
                           fet1_customer_credit.invoice_id
                          )
                  ),
             12,
             ' '
 ...
 WHERE ba.bill_seq = ${billSeq}
   ...
   AND ba.bill_seq = fet1_customer_credit.bill_seq_no
   ...
   AND fet1_customer_credit.credit_reason NOT IN (
                                                SELECT refund_reason_code
                                                 ...
                                                 WHERE reversal_indicator =
                                                                           'Y')
   AND fet1_customer_credit.credit_reason =
                                         fet1_credit_reason.credit_reason_code
   AND (   (    fet1_credit_reason.category_code = 'D'
            AND fet1_customer_credit.credit_reason IN ('DP-0', 'DP-8')
           )
